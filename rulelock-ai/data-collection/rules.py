"""
RuleLock AI — Business Rule Validation Engine
Owner: Sadini
...
Merged in from the standalone rule-engine service so the checkout review
pipeline runs in a single process instead of over HTTP between services.
"""
...
class RuleResult:
    passed: bool
    reason: str = ""
    # Machine-readable code matching enforcement's RULE_ACTION_MAP keys
    # (decision.py) — this is what lets the enforcement step pick the right
...
def validate_transaction(transaction: dict) -> dict:
    ...
    Returns:
        ...
        rule_code: str | None    — code of the FIRST failing rule, ready to pass
                                    straight to the enforcement decision step
    """
