...
async function checkServices() { try { await request('/health'); byId('pipeline-status').textContent = 'Data layer online'; byId('api-origin').textContent = `API: ${new URL(API_BASE).host}`; } catch { byId('pipeline-status').textContent = 'Demo mode · API offline'; byId('api-origin').textContent = `API unavailable · ${new URL(API_BASE).host}`; } }
